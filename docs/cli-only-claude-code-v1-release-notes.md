...
CLI-only Claude Code v1 is the first productized Cairn Memory path for one local
...
- Claude Code hook capture through `cairn hook`.
- Safety gate through `cairn audit secrets`.
- Ingestion through `cairn ingest`.
- Behavior evaluation through `cairn eval run` and `cairn eval dogfood`.
- Read-only consolidated dogfood review through `cairn dogfood`.
- User-marked outcomes through `cairn outcome mark` and
...
- Verification preflight through `cairn verify`.
- Deterministic memory rendering through `cairn render`.
- Claude memory injection through `cairn inject claude`.
...
- `cairn audit secrets` passed after the outcome write
