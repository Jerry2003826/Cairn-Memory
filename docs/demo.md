...
Run this only in a sandbox repository created by `scripts/create_sandbox.sh`. Do not test on a real project until `cairn audit secrets` exits 0.
...
- `CLAUDE.md` contains only this managed region added by Cairn Memory:
...
Start a fresh Claude Code session in the same sandbox after `cairn inject claude --mode link`.
...
Inspect the run with `cairn run show <run_id>` and, if needed, expand relevant events with:

```bash
cairn run show <run_id> --seq <seq>
```

Use the `command` column in `cairn run show <run_id>` to identify the first Bash test command and any rediscovery reads before it. Record the first matching test command and any rediscovery events in `docs/spike-report-template.md`.
...
Expected result: `cairn verify` selects the active `uses_test_command` fact and
...
This is the approved write bridge. `cairn verify` itself remains SQLite read-only
...
acceptance above. Run it in the same sandbox only after `cairn audit secrets`
passes.

Before starting a fresh agent run:

```bash
cairn task start "manual task lifecycle demo" --task-type validation
```
...
cairn task status
cairn task read
cairn task ls --status open
```

Expected results:

- `cairn task status` reports one open task with at least one attached run.
- `cairn task read` returns leak-free JSON for the current project's open task
  only; it must not expose internal ids, timestamps, evidence, or confidence.
- The read-only commands above do not migrate or write Cairn Memory state.

Close the task with explicit outcome status:

```bash
cairn task close --success --from-verify
```
...
stopped without recording an outcome, use `cairn task abandon --reason "<why>"`
...
Expected result: `cairn audit secrets` exits 0, and no file under `.omni/`
...
- [ ] G1 hook capture writes only redacted data and `cairn hook` exits 0.
...
- [ ] G4 `cairn audit secrets` scans the full `.omni/` tree and exits 0.
