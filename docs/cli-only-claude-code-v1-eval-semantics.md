...
| field | meaning |
|---|---|
| `claude_md_read` / `memory_md_read` | a `CLAUDE.md` / `.omni/generated/memory.md` reference was observed in run events |
...
| `memory_effect` | when |
|---|---|
| `helped` | an expected command ran with `rediscovery_count == 0` **and** memory context (`CLAUDE.md`/`memory.md`) was observed |
...
Key rule: without observable memory-read evidence, a clean run is classified
`neutral`, never `helped`. Claude Code often imports `CLAUDE.md` / `memory.md`
without emitting a detectable `Read` event, so `neutral` is expected even when
