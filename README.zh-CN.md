...
omni inject opencode --mode preview    # 预览 opencode.json instructions
...
| | `omni inject opencode --mode preview\|link` | — | 将 `.omni/generated/memory.md` 加进项目本地 `opencode.json` instructions |
| **捕获** | `omni hook` *(自动调用)* | — | 脱敏 hook 输入并追加到 spool；永远退出 `0` |
| | `omni ingest [--engine claude\|opencode]` | W | 把脱敏 trace 导入本地存储 |
...
Phase C C-2 已在此分支实现 OpenCode v0。它加入项目本地
`opencode.json` instruction 注入，以及 UTF-8 `opencode run --format json`
transcript ingest；不会加入插件后台进程、MCP server 或新迁移。C-2 剩余
acceptance 步骤是 dogfood closeout 证据。
...
| **② OmniBridge** | agent-agnostic 的 capture/inject 接缝与只读机器访问接口；OpenCode v0 作为第一个第二引擎证明；MCP wrapper 基于它继续做 | ✅ 基础已落地；C-2 代码已落地、dogfood 待完成；C-4 待做 |
...
`opencode.json` instructions 和 UTF-8 JSONL transcript ingest；C-2 之后的下一个
证明点是在现有只读接口上包一层只读 MCP。
