...
| 只读的 `cairn verify`、`verify plan`、memory read、failure read | 向量 / embedding 检索 |
...
| **② Cairn Bridge** | agent-agnostic 的 capture/inject 接缝与只读机器访问接口；OpenCode v0 作为第一个第二引擎证明；MCP wrapper 基于它继续做 | ✅ 基础已落地；C-2 已交付；C-4 待做 |
