...
cairn inject qwen --mode preview        # 预览 QWEN.md managed 区块
cairn inject qwen --mode link           # 把 memory.md 链接进 QWEN.md
```
...
| | `cairn inject qwen --mode preview\|link` | — | 管理项目本地 `QWEN.md` managed 区块 |
| **捕获** | `cairn hook` *(自动调用)* | — | 脱敏 hook 输入并追加到 spool；永远退出 `0` |
| | `cairn ingest [--engine claude\|opencode\|qwen]` | W | 把脱敏 trace 导入本地存储 |
...
transcript ingest；不会加入插件后台进程或新迁移。QwenCode v0 现在加入项目本地
`QWEN.md` managed 区块注入，以及 UTF-8 `qwen --output-format stream-json`
transcript ingest，仍然不加后台进程或新迁移。C-4 现在加入 `cairn mcp serve`，
把现有机器读取接口包装成只读 stdio MCP 工具。

2026-06-16 的 Phase C 交付证据见
[`docs/phase-c-final-delivery-2026-06-16.md`](docs/phase-c-final-delivery-2026-06-16.md)：
五个有边界的 OpenCode 样本在 validation、release build、bugfix、refactor 和
known-failure recovery 中读取 Cairn Memory surface，并通过已批准 CLI writer
留下 `engine=opencode` 的 run、task、outcome 证据。
...
| QwenCode v0 `QWEN.md` 注入与 transcript ingest | QwenCode plugin/后台捕获或全局 `~/.qwen` 改动 |
...
| **② Cairn Bridge** | agent-agnostic 的 capture/inject 接缝与只读机器访问接口；OpenCode v0 作为第一个第二引擎证明；QwenCode v0 加入下一类窄 adapter；C-4 把读取接口包装成 MCP 工具 | ✅ 基础已落地；C-2 已交付；QwenCode v0 已交付；C-4 已交付 |
...
`opencode.json` instructions 和 UTF-8 JSONL transcript ingest。QwenCode v0
使用项目本地 `QWEN.md` managed 区块注入和 `qwen --output-format stream-json`
transcript ingest。`cairn mcp serve` 提供 `memory_read`、`failure_read`、
`verify_plan` 和 `task_read` 四个只读 stdio MCP 工具。

客户 pilot 可以先跑不需要真实 agent/API key 的试用包：

```powershell
bash scripts/customer_trial_demo.sh
```

它会创建一次性 sandbox，用 synthetic transcript 跑通 governed memory loop，
验证只读 MCP、执行 secrets audit，并在 `.customer-trial/report.json` 写出简洁报告。
详见 [`docs/customer-trial.md`](docs/customer-trial.md)。
...
- [`docs/customer-trial.md`](docs/customer-trial.md) —— 客户 pilot runbook 与 no-agent 试用包
