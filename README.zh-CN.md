...
Phase C C-2 已批准并正在实现 OpenCode v0。该分支会加入项目本地
...
| **② OmniBridge** | agent-agnostic 的 capture/inject 接缝与只读机器访问接口；OpenCode v0 作为第一个第二引擎证明；MCP wrapper 基于它继续做 | ✅ 基础已落地；C-2 进行中；C-4 待做 |
...
今天 Claude 仍是唯一已安装的 hook capture 目标。OpenCode v0 会使用项目本地
